...
public class LGHorizontalLinearFlowLayout: UICollectionViewFlowLayout {
    ...
    public var scalingOffset: CGFloat = 150 //for offsets >= scalingOffset scale factor is minimumScaleFactor
    ...
    private func configureInset() -> Void {
        guard let collecitonView = self.collectionView else { return }
        let inset = self.collectionView!.bounds.size.width / 2 - self.itemSize.width / 2
        collecitonView.contentInset = UIEdgeInsetsMake(0, inset, 0, inset)
        collecitonView.contentOffset = CGPoint(x: -inset, y: 0)
    }
...
//            transform = CATransform3DTranslate(transform, 1, -50, 1)
            newAttributes.transform3D = transform
        }
